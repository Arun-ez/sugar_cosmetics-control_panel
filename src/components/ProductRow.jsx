...
const ProductRow = ({ data, load }) => {
    ...
    const switch_onchange = async () => {
        ...
        let response = await Axios.patch(`/products/${_id}`, { visibility: !visibility });
    ...
    return (
        <Tr whiteSpace={'nowrap'} bg={inventory <= 0 ? 'red.600' : ''}>
            ...
            <Td>
                ...
                <ProductDrawer isOpen={isOpen} onClose={onClose} data={data} load={load} type='update' />
            </Td>
        </Tr>
    )
}
